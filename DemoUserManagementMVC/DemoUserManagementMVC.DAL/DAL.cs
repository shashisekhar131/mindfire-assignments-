        ...
        private IQueryable<Note> ApplySorting(IQueryable<Note> Query, string SortExpression, string SortDirection)
        {
            switch (SortExpression)
            {
                ...
                case "ObjectType":
                    ...
                    break;
                case "CreatedDate":
                    Query = (SortDirection == "ASC") ? Query.OrderBy(n => n.CreatedDate) : Query.OrderByDescending(n => n.CreatedDate);
                    break;
        ...
        private IQueryable<Document> ApplySorting(IQueryable<Document> Query, string SortExpression, string SortDirection)
        {
            switch (SortExpression)
            {
                ...
                case "DocumentOriginalName":
                    Query = (SortDirection == "ASC") ? Query.OrderBy(d => d.DocumentOriginalName) : Query.OrderByDescending(d => d.DocumentOriginalName);
                    break;
                case "TimeStamp":
                    Query = (SortDirection == "ASC") ? Query.OrderBy(d => d.TimeStamp) : Query.OrderByDescending(d => d.TimeStamp);
                    break;
